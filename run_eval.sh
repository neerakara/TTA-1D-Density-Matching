...
# set default values here. specific values for different TTA methods are set in the following lines
...
tta_method='FoE' # FoE / AE / entropy_min / DAE
...
if [[ "$eval_type" -eq 1 ]]; then
    ...
    tta_method='DAE'
    lr=0.001
    stop='best_dice' # choose this option in the for loop below
...
for stop in 'best_dice' # 'best_dice' 'best_loss' # 'tta_init' 'best_loss_in_first_10_epochs' 'best_loss_in_first_100_epochs' # 0 | 10 | 100 | 1000
do

    for ts_dataset in 'UMC' 'NUHS' # 'BMC' 'USZ' 'UCL' 'HK' 'BIDMC' 'UHE' 'HVHD' 'site1' 'site3' 'site4' 'NUHS' 'CALTECH' # | 'RUNMC' 'CSF' 'UMC' 'site2' 'HCPT1' 'CALTECH'
    do   
